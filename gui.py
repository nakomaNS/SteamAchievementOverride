...
class App(QMainWindow):
    ...
    def create_loading_panel(self):
        ...
        gradient_style = """
            QWidget#loading_panel {
                ...
                /* Cor de início (quase preto, ligeiramente mais claro) */
                /* Cor de fim (quase preto, ligeiramente mais escuro) */
            }
        """
    ...
    def on_game_select(self, appid, game_name):
        ...
        if appid_str in self.app_validation_cache:
            if self.app_validation_cache[appid_str]: 
                ...
                self.show_custom_message('info', 'no_achievements', 'info', self.current_game_name) # Mostra pop-up de "sem conquistas"
        else:
    ...
    def set_language(self, lang):
        """Define o idioma da aplicação e atualiza a UI."""
    ...
    def _fetch_all_images_parallel(self, games_to_fetch):
        """Baixa imagens de jogos em paralelo."""
    ...
    def _show_achievements_for_valid_game(self):
        """Prepara e exibe o painel de conquistas para um jogo válido."""
    ...
    def display_achievements(self, achievements_to_display):
        """Exibe as conquistas na interface."""
        for i in reversed(range(self.ach_list_layout.count())): 
        ...
        for ach in sorted(achievements_to_display, key=lambda x: x['name'].lower()): 
    ...
    def create_achievement_widget(self, ach_data, app_instance): 
        """Cria um widget individual para uma conquista."""
        ...
        if 'icon_base64' in ach_data and ach_data['icon_base64']:
            try:
                ...
                if not pixmap.isNull():
                    ...
                    print(f"Erro: Não foi possível carregar imagem para {ach_data['apiName']} de dados Base64 (formato inválido após todas as tentativas).")
            except Exception as e:
                print(f"Erro ao processar ícone de conquista (Base64) para {ach_data['apiName']}: {e}")
        else:
            icon_label.setText("?")
            icon_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            icon_label.setStyleSheet("background-color: #444; border-radius: 4px; color: #bbb; font-size: 24pt; font-weight: bold;")
    ...
    def _unlock_achievements_worker(self, to_unlock, appid_to_refresh):
        try:
            ...
            if result.returncode != 0:
                error_message = f"Falha ao executar o desbloqueador (código {result.returncode}).\n"
                if result.stdout:
                    error_message += f"Saída Padrão:\n{result.stdout}\n"
                if result.stderr:
                    error_message += f"Erro Padrão:\n{result.stderr}"
